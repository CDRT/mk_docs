...
# Initialize the module (required first step)
Initialize-LnvThinkBiosConfig

# View all current BIOS settings
Show-LnvWmiSettings
```
...
??? note "Backup current configuration"
    ```powershell
...
??? note "Change individual settings"
    ```powershell
...
??? note "Create deployment package"
    ```powershell
...
??? note "Initialize-LnvThinkBiosConfig"
...
??? note "Read-LnvTBCPreferenceFile"
...
??? note "Update-LnvTBCPreferenceFile"
...
??? note "Test-LnvTBCPendingChanges"
...
??? note "Export-LnvPasswordChangeFile"
...
??? note "Update-LnvPassword"
    ...
    **Syntax:**
    ...
    ```powershell
    Update-LnvPassword -Type <String> [-Old <SecureString>] [-New <SecureString>]
    ```

    **Description:**
    Prompts the user for the current and new password, then updates the specified BIOS password. Alternatively, passwords can be provided directly via the `-Old` and `-New` parameters.
    ...
    **Examples:**
    ...
    ```powershell
    # Update supervisor password with interactive prompts
    Update-LnvPassword -Type pap

    # Update power-on password interactively
    Update-LnvPassword -Type pop

    # Update with provided passwords
    $curPw = Read-Host -AsSecureString 'Current password'
    $newPw = Read-Host -AsSecureString 'New password'
    Update-LnvPassword -Type pap -Old $curPw -New $newPw
    ```

    **Interactive Prompts:**

    - Current password (if `-Old` parameter not provided)
    - New password (if `-New` parameter not provided)
    - Password confirmation (if `-New` parameter not provided)
    ...
    **Notes:**
    ...
    - New password must match confirmation (if entered interactively)
...
??? note "Submit-LnvFunctionRequest"
...
- Initial module development with core cmdlet functionality for WMI BIOS settings management
