...
The module is published in the PowerShell Gallery so it can easily be installed with the Install-Module command. To install from the PowerShell Gallery, simply run
...
: #### Description
    ...
    Parse and analyze Lenovo Commercial Vantage System Update Addin log files, producing a color-coded, section-by-section summary including session info, scan results, per-package install metrics, errors, and warnings. Supports both the current and legacy log formats, optional output to file, section filtering, and benign-warning suppression.
    ...
    **IncludeBenign**
    ...
    ``` PowerShell
    Invoke-LCVLogViewer -LogFile ".\session.log" -Sections SystemInfo,InstallResults
    ```

    Displays only the System Info and Install Results sections.

    ``` PowerShell
    Invoke-LCVLogViewer -LogFile ".\session.log" -OutputFile ".\summary.txt" -Force -IncludeBenign
    ```
...
: #### Description
    ...
    Analyzes log files generated by Lenovo ThinInstaller and provides a human-readable summary of key session details including system information, applicable updates, installation results, and errors.
    ...
    **NoColor**
    ...
    ``` PowerShell
    Invoke-LnvTILogFileViewer -LogFile "C:\Logs\Update_log_250221164004.txt"
    ```

    Analyzes the specified log file.

    ``` PowerShell
    Invoke-LnvTILogFileViewer -LogDirectory "C:\Logs" -OutputFile "C:\Logs\summary.txt"
    ```
