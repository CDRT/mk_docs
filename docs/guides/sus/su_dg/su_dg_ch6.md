...
_Table 6-1. Advanced System Update Registry settings_
...
| --- | --- | --- |
| **Connection** | Allow Proxy Fail to Direct | When enabled, if the proxy server cannot be reached, then System Update will attempt a direct connection instead. If disabled and the proxy server cannot be reached, System Update will present an error message and stop. |
...
| | Proxy/AutoConfigURL <br> _Refers to the number of different proxy servers that may be encountered. The count must not exceed five._ | This setting stores the automatic configuration Uniform Resource Locator (URL) path such as `file://c:/Proxy1.pac` or `http://10.10.1.1/Proxy1.pac`. |
| | Proxy/ProxyServer <br> _Refers to the number of different proxy servers that may be encountered. The count must not exceed five._ | This setting stores the proxy server location such as `http=10.10.1.1:8080` or `socks=10.10.1.1:1080`. |
...
_Table 6-2. Computer Configuration \> Administrative Templates \> ThinkVantage \> System Update \> User Settings._
...
_Table 6-3. \UserSettings\Connection_
